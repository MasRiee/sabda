"""
server.py — Web dashboard untuk pendamping SABDA
"""
...
session_stats = {
    ...
    "history": [],         # [{time, target, predicted, correct, confidence}]
...
def broadcast_update(data: dict):
    """Dipanggil dari AppController saat ada prediksi baru."""
...
# Tambahkan di server.py setelah definisi session_stats
...
def inject_dummy_history():
    ...
    for days_ago in range(6, 0, -1):
        ...
        # Simulasi progres meningkat dari hari ke hari
        base_rate = 0.40 + (days_ago * -0.04)  # makin baru makin tinggi
        ...
        # Update word_stats dari data historis
    ...
    for target, predicted, correct, conf in sample_today:
        ...
        # Perbaiki timestamp riwayat supaya terlihat natural
...
def on_reset():
    """Reset statistik session (dipanggil dari tombol di dashboard)."""
    session_stats["total_attempts"] = 0
    session_stats["total_correct"] = 0
    session_stats["word_stats"] = {}
    session_stats["history"] = []
    # daily_stats tidak direset — biar tetap ada riwayat harian
